...
kpx = 1; % constante kpx do controlador 
kpy = 1; % constante kpy do controlador
...
rt = 1; % raio da trajetória
vt = 0.25; % velocidade linear
wt = vt/rt; % velocidade angular

xd = rt*cos(wt*t); % x desejado da trajetória em função do tempo
yd = rt*sin(wt*t); % y desejado da trajetória em função do tempo

xD = [xd;yd];   % variável com todo os trajeto

xd_ponto=diff(xd)/ts; % derivada temporal de x da trajetória
xd_ponto(length(t)) = xd_ponto(length(t)-1);
yd_ponto=diff(yd)/ts; % derivada temporal de y da trajetória
yd_ponto(length(t)) = yd_ponto(length(t)-1);
...
save('Tarefa1_Pioneer','pose','xD','vRobot','vD','pErro','IAEp','vErro','IAEv')
